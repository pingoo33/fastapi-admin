...
async def retrieve_user(user_id, user_repository: UserRepository):
...
@inject
async def get_subject(access_token: str, refresh_token: str,
    ...
    try:
        ...
        return AuthenticationSubject(await retrieve_user(payload_access['user_id'], user_repository),
                                     "", payload_access['exp'],
                                     "", payload_refresh['exp'])
        ...
        try:
            ...
            return AuthenticationSubject(await retrieve_user(payload['user_id'], user_repository),
...
class JWTProvider:
    async def __call__(self,
        ...
        return await get_subject(access_token, refresh_token)
